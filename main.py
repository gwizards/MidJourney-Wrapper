...
async def mj_upscale(ctx, index: discord.Option(int), reset_target : discord.Option(bool) =True):
    ...
    if Globals.targetID == "":
        await ctx.respond('You did not set target. To do so reply to targeted message with "$mj_target"')
        return
...
# Improved readability by removing unnecessary code, improved performance by removing unnecessary imports, removed unnecessary variable